...
let inputAxis = Axis.OperandSequenceAxis()//new Axis("inputAxis")
let labelAxis = Axis.OperandSequenceAxis()//new Axis("labelAxis")
...
let create_model =
  ...
  let attention_model = M.AttentionModel(D attention_dim, name="attention_model")
...
let train 
  ...
  let ce,errs = create_critetion_function model_train x y
  
  let model = model_train x y
